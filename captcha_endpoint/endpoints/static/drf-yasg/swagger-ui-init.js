...
function initSwaggerUi() {
    ...
    var swaggerConfig = {
        ...
        requestInterceptor: async function (request) {
            ...
            var site_captcha_token = document.querySelector("[name=sitecaptchatoken]");
            if (csrftoken) {
                headers["X-CSRFToken"] = csrftoken.value;
            }
            console.log(site_captcha_token.value)
            grecaptcha.ready(function() {
              grecaptcha.execute(site_captcha_token.value, {action: 'submit'}).then(function(token) {
                  headers["captcha-security"] = token;
                  console.log(token)
              });
            })
